...
class SharingHandler(tornado.web.RequestHandler):
    ...
    def post(self, user_id, collection_name):
        ...
        self.__log.info('%s - POST: create sharing space for collection %s for user %s' % (
            str(self.__class__), collection_name, user_id))
    ...
    def delete(self, user_id, collection_name):
        ...
        self.__log.info('%s - DELETE: delete sharing space for collection %s for user %s' % (
            str(self.__class__), collection_name, user_id))
    ...
    def get(self, user_id, collection_name):
        ...
        self.__log.info('%s - GET: Get sharing space info for collection %s for user %s' % (
            str(self.__class__), collection_name, user_id))
        ...
        if does_exist:
            ...
            if sharing_record is not None:
                ...
                if sharing_secret is None:
                    self.set_status(StorageResponse.NOT_FOUND)
                    self.__log.warning("GET-SharingSpace- No sharing record exists for user %s and collection %s" % (
                        user_id, collection_name))
                    self.finish()
                else:
