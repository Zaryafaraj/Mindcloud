...
class CollectionFileHandler(tornado.web.RequestHandler):
    ...
    def delete(self, user_id, collection_name, file_name):
        ...
        collection_name = urllib2.unquote(collection_name)
        file_name = urllib2.unquote(file_name)
        result_code = yield gen.Task(StorageServer.delete_collection_file, user_id, collection_name, file_name)
        self.set_status(result_code)
        self.finish()

    @tornado.web.asynchronous
    @gen.engine
    def post(self, user_id, collection_name, file_name):
        ...
        if len(self.request.files) > 0:
            ...
            result_code = yield gen.Task(StorageServer.save_collection_file, user_id, collection_name, collection_file)
            self.set_status(result_code)
            self.finish()
        else:
