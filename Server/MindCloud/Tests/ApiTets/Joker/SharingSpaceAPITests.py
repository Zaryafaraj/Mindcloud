...
class SharingSpaceTests(AsyncHTTPTestCase):
    ...
    @staticmethod
    def __make_update_not_action(user_id, collection_name, note_name):
    ...
    @staticmethod
    def __make_update_note_img_action(user_id, collection_name, note_name):
    ...
    @staticmethod
    def __make_update_thumbnail_action(user_id, collection_name):
    ...
    def test_add_listener(self):
        ...
        sharing_space_url = '/'.join(['', 'SharingSpace', sharing_secret, 'Listen', owner_id])
    ...
    def test_add_listener_missing_parameter(self):
        ...
        #TODO- Make sure API returns 400 for these
        self.assertEqual(StorageResponse.SERVER_EXCEPTION, response1.code)
        self.assertEqual(StorageResponse.SERVER_EXCEPTION, response2.code)
    ...
    def test_get_temp_img_thumbnail(self):
        ...
        temp_url = sharing_space_url + '/' + owner_id + '/' + collection_name + '/Thumbnail/' + secret
    ...
    def test_get_temp_img_note_img(self):
        ...
        for note_name in details:
            ...
            temp_url = sharing_space_url + '/' + owner_id + '/' + collection_name + '/' + note_name + '/' + secret
            response = self.fetch(path=temp_url, method='GET')
            self.assertEqual(StorageResponse.OK, response.code)
            self.assertTrue(response.body is not None)
    ...
    def test_get_temp_img_invalid_secret(self):
        ...
        for note_name in details:
            ...
            #and use other information to get the image
            self.assertEqual(StorageResponse.OK, response.code)
            self.assertTrue(response.body is not None)
    ...
    def test_get_temp_img_invalid_parameters(self):
        ...
        details = self.__primary_listener_json_obj[SharingEvent.UPDATE_NOTE_IMG]
        for note_name in details:
            secret = 'bad_secret' + note_name
            temp_url = sharing_space_url + '/' + owner_id + '/' + 'dum_col' + '/' + 'non_existing_dummy' + '/' + secret
            response = self.fetch(path=temp_url, method='GET')
            self.assertEqual(StorageResponse.NOT_FOUND, response.code)
